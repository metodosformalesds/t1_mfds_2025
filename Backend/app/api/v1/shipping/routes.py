...
from app.api.v1.shipping import service
...
def crear_pedido(pedido_in: CreateOrder, db: Session = Depends(get_db)):
    ...
    try:
        nuevo_pedido = service.create_order_db(db=db, order_in=pedido_in)
...
def rastrear_pedido( pedido_id: int, db: Session = Depends(get_db)):
    tracking_details = service.get_details(db=db, pedido_id=pedido_id)
